...
def strip_native_caption(window, log=None):
    """Remove WS_CAPTION without subclassing (safe). Leaves a possible thin resize edge."""
    ...
    try:
        ...
        # Win10 1903 / Win10 2004+ immersive dark mode attribute ids
            ...
            if handle is not None:
                if hasattr(handle, 'ToInt64'):
                    ...
                    try:
                        ...
                        raw = 0
        hwnd = int(user32.GetAncestor(raw, GA_ROOT) or 0) if raw else 0
        if not hwnd:
            _log(f'strip_native_caption: hwnd not found raw={raw}')
            return False
        ...
        try:
            ...
            # -1 on all sides: DWM glass covers the full window so residual
            # non-client paint (the top black edge) is not drawn as a separate bar.
            ctypes.windll.dwmapi.DwmExtendFrameIntoClientArea(
                hwnd, ctypes.byref(MARGINS(-1, -1, -1, -1))
            )
        except Exception as dwm_err:
            _log(f'strip_native_caption: DwmExtend skip: {dwm_err}')
        try:
            # Match app light chrome; kills the black residual NC band on Win11.
            dark = ctypes.c_int(0)
            ...
            for attr in (DWMWA_USE_IMMERSIVE_DARK_MODE, DWMWA_USE_IMMERSIVE_DARK_MODE_BEFORE):
                try:
                    ...
                    pass
            try:
                # Win11 22H2+: hide residual resize border (the top black edge).
            ...
            _log(f'strip_native_caption: DwmDark skip: {dark_err}')
        _log(f'strip_native_caption: ok hwnd={hwnd}')
        return True
    except Exception as exc:
        _log(f'strip_native_caption failed: {type(exc).__name__}: {exc}')
        return False
...
class AppHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_import(self):
        ...
        if isinstance(body, dict) and isinstance(body.get('plant'), dict) and body['plant'].get('plant') is not None:
            try:
                plant_payload = body['plant']
                if 'schemaVersion' not in plant_payload:
                    plant_payload = {
                        'schemaVersion': 1,
                        'plant': plant_payload.get('plant'),
                        'updatedAt': datetime.now().isoformat(),
                    }
                save_plant(plant_payload)
                plant_restored = True
            except Exception:
                plant_restored = False
        self.send_json({
    ...
    def serve_image(self, relative_path):
        ...
        filepath = os.path.join(DATA_DIR, decoded_path.replace('/', os.sep))
        if os.path.exists(filepath):
            ext = os.path.splitext(filepath)[1].lower()
            content_types = {
                '.png': 'image/png',
    ...
    if HAS_WEBVIEW:
        try:
            ...
            def _on_shown(*_args, **_kwargs):
                ...
                # Win11 WebView2 may re-apply caption after first frame; retry a few times.
            ...
            try:
                ...
                window_api.bind_window(window)
                def _on_shown_fb(*_a, **_k):
                    def _attempt_fb():
                        strip_native_caption(window, log=write_log)
                    for delay in (0.0, 0.15, 0.4, 0.9):
                        timer = threading.Timer(delay, _attempt_fb)
                        timer.daemon = True
                        timer.start()

                try:
                    window.events.shown += _on_shown_fb
