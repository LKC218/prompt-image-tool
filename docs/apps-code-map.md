...
- `src/js/pc-welcome-banner.js`：PC 端顶部欢迎横幅共享渲染模块，首页、提示词库、新建/编辑、分类与标签页面共用，并支持编辑页返回与保存动作区布局。
- `src/js/pc-home.js`：PC 端首页仪表盘渲染和交互入口，承载欢迎横幅、悬浮搜索、统计卡、最近使用、收藏分类、快速创建和本地安全提示。
- `src/js/pc-library.js`：PC 端提示词库页面渲染和交互入口，承载搜索筛选、表格列表、选中预览、分页、收藏、复制和提示词更多操作。
- `src/js/pc-editor.js`：PC 端新建/编辑提示词页面渲染和交互入口，承载标题、提示词、标签、分类、比例、图片预览与保存流程。
- `src/js/pc-category.js`：PC 端分类与标签页面渲染和交互入口，承载分类表格、标签表格、拖拽排序、快捷操作和本地图标引用。
- `src/js/pc-settings.js`：PC 端设置页渲染与交互入口，承载外观设置、本地存储、数据备份恢复和局域网同步操作。
- `src/css/pc.css`：PC 端全局样式与页面专用样式，首页样式集中在 `pc-home-*` 类名，提示词库样式集中在 `pc-library-*` 类名，设置页布局样式集中在 `pc-settings-*` 类名，分类与标签页样式集中在 `pc-category-*`、`pc-tag-*` 和 `pc-quick-*` 类名。
...
- `docs/项目开发经验/`：可复用工程规范、问题经验和跨页面实施约定。
