...
Android 点击同步
  ...
  → 拿到 { prompt_sets, versions, images }
  → 冲突检测与合并
  → 写入本地 SQLite
...
同一个 prompt_set 在 PC 和 Android 两端都被修改过。
...
→ Android 的版本更新，覆盖 PC 的
```

- 按 `updated_at` 字段对比，取最新的
- 实现最简单，个人工具够用
- 缺点：晚改的不一定是对的，可能误覆盖

### 5.3 优化版：合并 + 冲突副本

**规则**：不覆盖，合并。以 prompt_set 为最小合并单位。

```
PC 有：集合 A(改过)、集合 B(改过)、集合 C
Android 有：集合 A(改过)、集合 B、集合 D(新增)

合并结果：
  A → 两边都改过 → 冲突，保留两个版本（A 和 A'）
  B → 只 PC 改过 → 用 PC 的
  C → 只 PC 有 → 同步过来
  D → 只 Android 有 → 保留
```
...
| 两端都有，两端都改过 | **冲突**：保留两个，重命名其中一个为 `集合名 (冲突副本)` |

**冲突副本命名规则**：

```
原集合名：我的集合
冲突副本：我的集合 (冲突副本 2026-05-03 15:00)
```
...
对比每个 prompt_set 的 `updated_at` 是否与 PC 端一致（冲突副本除外）。
...
✅ 8 个版本同步成功
✅ 20 张图片同步成功
⚠️ 1 个冲突已保留副本：我的集合 (冲突副本 2026-05-03 15:00)
❌ img15.png 下载失败（可重试）
```
...
| 误操作覆盖数据 | 无确认机制 | 同步前弹窗确认，显示将要变更的内容 |
