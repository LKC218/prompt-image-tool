...
| ID | 问题 | 位置 | 影响 | 根因 | 优化方向 |
|----|------|------|------|------|----------|
| A1 | 搜索无 debounce，每次按键全量重渲染 | `src/js/pc-library.js:541-545`、`pc-home.js:341-347`、`pc-category.js`、`mobile-library.js` | 打字卡顿、表格闪烁 | `input` 直接调 `render*`；全仓库 **0 处 debounce** | 150–200ms debounce；仅更新 tbody/列表节点 |
| A2 | 库页每次渲染重建整个工作区 DOM | `src/js/pc-library.js:272-301` | 选中态、滚动位置、预览区反复丢失/重绘 | `container.innerHTML = table + preview` 一体重建 | 表格区与预览区分轨更新；差分行 |
...
| A4 | 移动端库无分页，全量渲染 + 逐项动画延迟 | `src/js/mobile-library.js:124-155` | 几百条时首屏 DOM 爆炸、滚动掉帧 | `list.map` 一次渲染全部；`animation-delay: idx * 30ms` | 分页/滚动加载；长列表关闭 stagger |
...
| ID | 问题 | 位置 | 影响 | 根因 | 优化方向 |
|----|------|------|------|------|----------|
| B1 | PC 壳静态 import 全部页面模块 | `src/js/pc-app.js:15-25` | 启动解析/执行成本与功能无关（含 tetris/plane/goals） | 路由页未代码分割 | 路由级 `import()`；游戏/编辑器/设置懒加载 |
...
| ID | 问题 | 位置 | 影响 | 根因 | 优化方向 |
|----|------|------|------|------|----------|
| D1 | `getPromptSets` 对每条 set 额外 2–3 次查询 | `src/js/sqlite-storage.js:232-253` | 列表加载随条目线性变慢（N+1） | 循环内 COUNT + first image | 一条聚合 SQL / JOIN 一次出摘要 |
