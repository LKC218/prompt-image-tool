...
| `exportData()` | SELECT 所有表 | 导出全部数据 |
| `importData(data)` | INSERT OR IGNORE | 导入数据（跳过已存在的集合） |
...
3. 拉取数据：GET http://IP:8888/api/sync
4. 冲突检测与合并（基于 updated_at 时间戳）
5. 写入本地 SQLite
6. 逐个下载图片：GET http://IP:8888/api/sync/images/{filename}
7. 校验数据完整性
8. 显示同步报告
```
...
| 仅 PC 端有 | 同步到 Android |
| 仅 Android 端有 | 保留不动 |
| 两端都有，仅 PC 改过 | 用 PC 的版本覆盖 |
| 两端都有，仅 Android 改过 | 保留 Android 的 |
| 两端都有，两端都改过 | **冲突**：保留两个，重命名为 `集合名 (冲突副本 时间)` |
...
| 数据导入导出 | JSON 格式，仅导入新集合（不覆盖） | `app.js` → `SqliteStorage` → SQLite |
...
| 局域网同步 | 从 PC 端拉取全量数据 | `lan-sync.js` → `LanSync.sync()` |
...
| AND-002 | 导入数据时不覆盖已有集合，与 PC 端行为不一致 | 中 | 统一导入策略，支持覆盖更新 |
