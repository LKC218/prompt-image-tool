...
```bash
copy android\app\build\outputs\apk\release\app-release.apk releases\PromptImageManager-v2.3.2-Android.apk
```
...
releases/
└── PromptImageManager-v2.3.2-Android.apk   # 发布用副本
```
...
```text
releases/PromptImageManager-v2.3.2-Android.apk
```

APK 大小：`45025277` 字节。签名校验通过 APK Signature Scheme v2，签名者数量为 `1`。

2026-05-13 使用 `scripts/build_android_package.py --skip-env-check` 构建的 v2.3.2 发布副本：

```text
releases/PromptImageManager-v2.3.2-Android.apk
```

APK 大小：`46744828` 字节。`apksigner` 校验通过 APK Signature Scheme v2，签名者数量为 `1`；`aapt dump badging` 确认 `versionCode='7'`、`versionName='2.3.2'`。
...
| `android/app/build.gradle` | `versionCode` | `7`（每次 +1） |
| `android/app/build.gradle` | `versionName` | `"2.3.2"` |
