...
| 文件 | 当前值 | 位置 |
|------|--------|------|
| `package.json` / `package-lock.json` | `"version": "2.4.3"` | 第3行 / 第3、8行 |
| `src/index.html` | `<meta name="version" content="2.4.3">` | 第9行 |
| `build/installer.nsi` | `!define APPVERSION "2.4.3"` | 第2行 |
| `build.bat` | `v2.4.3` 与 `PromptImageManager-Setup-2.4.3.exe` | 构建标题与发布路径 |
| `src-tauri/tauri.conf.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` | `2.4.3` | 主桌面端配置与锁文件 |
| `installer-shell/` 下的包、Tauri 和 Cargo 文件 | `2.4.3` | 安装器壳配置与锁文件 |
| `android/app/build.gradle` | `versionCode 17` / `versionName "2.4.3"` | 第16-17行 |
