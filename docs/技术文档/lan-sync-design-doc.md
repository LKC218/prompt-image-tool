...
Android 上传本机快照，PC 只生成差异预览，不写入数据。预览会返回新增、跳过、冲突、无效项、PC 独有数量、字段级差异、推荐动作和 `requiresConfirmation`。移动端在回传、双向同步以及拉取前使用该预览提示用户确认，避免静默覆盖。
...
冲突副本使用稳定 `conflictKey` 标识。`conflictKey` 由来源设备、原始记录 ID、业务内容摘要和同步方向生成；重复回传同一份冲突内容时，PC 会识别已有冲突副本并跳过，不再生成多份相同副本。冲突副本会写入 `syncMeta.sourceDeviceId`、`syncMeta.conflictKey`、`syncMeta.originalId` 和 `syncMeta.createdBySync`。

### 5.3 双向模式

双向同步由 Android 发起。当前流程为先调用 `/api/sync/preview` 生成冲突预览，用户确认后调用 `/api/sync/bidirectional`，PC 合并本机快照并返回最新同步快照，Android 再按返回的 `syncData` 写入本地。回传阶段默认沿用 `keep_pc`，拉取阶段沿用 PC 优先覆盖。

### 5.4 安全写入

PC 在执行普通导入、Android 回传或双向合并前，会在 `backups` 目录写入一份同步前备份。同步报告通过 `backupPath` 返回备份路径，便于失败后人工恢复。图片写入失败仍通过同步报告返回，不应删除既有 PC 数据。
