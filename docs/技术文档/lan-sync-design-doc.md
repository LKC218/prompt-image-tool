...
实现 Android 端与 PC 端在同一局域网内一键同步数据。PC 端作为服务端，Android 端作为客户端，通过 HTTP 协议完成数据与图片的传输。
...
- 同步过程有状态反馈，防止重复操作
- 同步完成后校验数据完整性
- PC 端作为唯一同步源，同 ID 数据以 PC 端为准覆盖移动端
...
Android 点击同步
  ...
  → fetch("http://192.168.x.x:8888/api/sync")
  → 拿到 { folders, prompt_sets, versions, images }
  → PC 优先覆盖合并
  → 写入本地 SQLite
  → 逐个下载图片 → 写入 Filesystem
  → 校验数据完整性
  → 同步完成
```
...
在 PC 端界面显示本机 IP 和端口，提示用户在 Android 端输入此地址进行同步。
...
同一个 prompt_set 在 PC 和 Android 两端都存在时，以 PC 端数据作为同步基准。
