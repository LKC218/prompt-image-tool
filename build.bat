...
echo.
echo ✅ PC 安装包构建完成！
echo 安装包：build\PromptImageManager-Setup-2.3.7.exe
if not exist releases mkdir releases
copy /Y build\PromptImageManager-Setup-2.3.7.exe releases\PromptImageManager-Setup-2.3.7.exe >nul
if %errorlevel% neq 0 (
    echo ⚠️ 安装包复制到 releases 失败，请手动复制
) else (
    echo 发布副本：releases\PromptImageManager-Setup-2.3.7.exe
)
echo.
goto menu
